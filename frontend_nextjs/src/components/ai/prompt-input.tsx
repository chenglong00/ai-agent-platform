...
export const PromptInput = ({
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = event => {
    ...
    /** Prefer provider state, then FormData, then direct textarea read (FormData misses fields under display:contents in some browsers). */
    const resolveMessageText = (): string => {
      if (usingProvider) {
        const fromState = controller.textInput.value
        if (fromState.trim()) return fromState
      }
      const formData = new FormData(form)
      const fromForm = (formData.get("message") as string) || ""
...
export const PromptInputBody = ({ className, ...props }: PromptInputBodyProps) => (
  <div
    className={cn(
      ...
      /* Avoid display:contents: it breaks FormData / field association for the message textarea in some browsers. */
      className,
    )}
    {...props}
  />
